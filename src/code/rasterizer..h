...
//                grid(y_coord, x_coord) += 1;
              ...
              Eigen::Matrix<uint32_t , 2, 1> point_grid = ((point2D - shift).cwiseQuotient(scale)+compensate).cast<uint32_t>();
        ...
        bool save_grid(const std::filesystem::path& path){
            ...
            file << "min_x: " << min_max[0] << "min_y: " << min_max[1] << "max_x: " << min_max[2] << "max_y: " << min_max[3] << "\n";
